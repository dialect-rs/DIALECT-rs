//! Numerical-derivative assertion harness, generic over any system type
//! implementing [`RestrictedSCC`](crate::scc_interface::RestrictedSCC).

use crate::scc_interface::RestrictedSCC;
use ndarray::prelude::*;
use ndarray::Slice;
...
/// Computes the numerical derivative of `function` at `origin` for coordinate `index`
/// using the 5-point central difference stencil: [-f(x+2h) + 8f(x+h) - 8f(x-h) + f(x-2h)] / (12h)
/// This has O(h^4) accuracy with a fixed step size, avoiding the noise amplification of Ridder's method.
fn five_point_stencil<S, F, D>(
    system: &mut S,
    function: F,
    origin: ArrayBase<D, Ix1>,
    index: usize,
    stepsize: f64,
) -> f64
where
    ...
    D: ndarray::Data<Elem = f64>,
{
    ...
    let f_p2h = function(system, &origin + &(&step * (2.0 * stepsize)));
    let f_p1h = function(system, &origin + &(&step * stepsize));
    let f_m1h = function(system, &origin - &(&step * stepsize));
    let f_m2h = function(system, &origin - &(&step * (2.0 * stepsize)));

    (-f_p2h + 8.0 * f_p1h - 8.0 * f_m1h + f_m2h) / (12.0 * stepsize)
}

/// Test the gradient of a function using the 5-point central difference stencil.
/// This provides O(h^4) accuracy with a fixed step size, making it more stable than
/// Ridder's method for larger molecular systems where SCF convergence noise is an issue.
/// Recommended stepsize: 1e-4 to 1e-5.
pub fn assert_deriv_5point<S, F, G>(
    system: &mut S,
    function: F,
    gradient: G,
    origin: Array1<f64>,
    ...
    G: Fn(&mut S) -> Array1<f64>,
{
    ...
    let mut string = format!(
        "{: <5} {: >18} {: >18} {: >18} \n",
        "Index", "Analytic", "Numerical", "Error"
    );
    for i in 0..origin.len() {
        let analytic_deriv: f64 = analytic_grad[i];
        let numerical_deriv: f64 =
            five_point_stencil(system, &function, origin.clone(), i, stepsize);
        let diff: f64 = (numerical_deriv - analytic_deriv).abs();
        error_values[i] = diff;

        string += &format!(
            "{: >5} {:>18.14} {:>18.14} {:>18.14} \n",
            i, analytic_deriv, numerical_deriv, diff
        );
    }
    let rmsd: f64 = (&error_values * &error_values).mean().unwrap().sqrt();
    let max: f64 = *error_values.max().unwrap();
    ...
    assert!(
        max < tol,
        "max |analytic - numerical| gradient deviation {:.3e} exceeds tolerance {:.1e}",
        max,
        tol
    );
}
...
/// Computes the numerical derivative of `function` at `origin` for coordinate `index`
/// using the 7-point central difference stencil:
/// f'(x) = [f(x+3h) - 9f(x+2h) + 45f(x+h) - 45f(x-h) + 9f(x-2h) - f(x-3h)] / (60h)
/// This has O(h^6) accuracy. Useful with tight SCF convergence (ε ~ 1e-12).
fn seven_point_stencil<S, F, D>(
    system: &mut S,
    function: F,
    origin: ArrayBase<D, Ix1>,
    index: usize,
    ...
    D: ndarray::Data<Elem = f64>,
{
    ...
    let f_p3h = function(system, &origin + &(&step * (3.0 * stepsize)));
    let f_p2h = function(system, &origin + &(&step * (2.0 * stepsize)));
    let f_p1h = function(system, &origin + &(&step * stepsize));
    let f_m1h = function(system, &origin - &(&step * stepsize));
    let f_m2h = function(system, &origin - &(&step * (2.0 * stepsize)));
    let f_m3h = function(system, &origin - &(&step * (3.0 * stepsize)));

    (f_p3h - 9.0 * f_p2h + 45.0 * f_p1h - 45.0 * f_m1h + 9.0 * f_m2h - f_m3h) / (60.0 * stepsize)
}

/// Test the gradient of a function using the 7-point central difference stencil.
/// This provides O(h^6) accuracy with a fixed step size. Most useful when SCF convergence
/// is tight (ε ~ 1e-12) and sub-1e-9 numerical accuracy is needed.
/// Recommended stepsize: 1e-2.
pub fn assert_deriv_7point<S, F, G>(
    system: &mut S,
    function: F,
    gradient: G,
    origin: Array1<f64>,
    ...
    G: Fn(&mut S) -> Array1<f64>,
{
    ...
    for i in 0..origin.len() {
        let analytic_deriv: f64 = analytic_grad[i];
        let numerical_deriv: f64 =
            seven_point_stencil(system, &function, origin.clone(), i, stepsize);
        let diff: f64 = (numerical_deriv - analytic_deriv).abs();
        error_values[i] = diff;
