use dialect_base::constants::HARTREE_TO_WAVENUMBERS;
use dialect_config::settings::WignerConfig;
use libm::tanh;
...
impl WignerEnsemble<'_> {
    /// Construct directly from a [`WignerConfig`] + atom count, without
    /// depending on the DFTB [`System`]. Lets AM1 (and any other backend)
    /// reuse the same Wigner sampler.
