//! Electronegativity Equalization (EEQ) model for initial charge guess
//!
//! This implements the EEQ model from xtb for computing initial atomic charges.
//! The model solves a linear system A·q = X with charge conservation constraint.
//!
...
/// sqrt(2/π) constant
const SQRT2PI: f64 = 0.7978845608028654;

// EEQ parameters for GFN0-xTB
// These are used for initial charge guess in GFN1-xTB
// Note: indices are atomic_number - 1
...
/// Coordination number scaling parameters
const EEQ_KAPPA: [f64; 86] = [
...
/// Charge width parameters
const EEQ_ALPHA: [f64; 86] = [
...
/// Compute initial atomic charges using the EEQ model
///
/// This solves the linear system A·q = X with charge conservation constraint:
/// - A[i,i] = gam[i] + sqrt(2/π) / alpha[i]  (diagonal: chemical hardness + self-screening)
/// - A[i,j] = erf(γij * r_ij) / r_ij          (off-diagonal: screened Coulomb)
/// - X[i] = -EN[i] + kappa[i] * sqrt(CN[i])   (RHS: electronegativity + CN correction)
pub fn compute_eeq_charges(atoms: &[XtbAtom], total_charge: f64) -> Array1<f64> {
    let n = atoms.len();
    ...
    // Compute coordination numbers (using the same function as for H0 matrix)
    let cn = calculate_coordination_numbers(atoms);

    // Build the augmented system (n+1 x n+1 matrix)
    // The last row/column is for the charge conservation constraint
    let m = n + 1;
    let mut amat = Array2::<f64>::zeros((m, m));
    let mut xvec = Array1::<f64>::zeros(m);

    // Build the RHS vector: X[i] = -EN[i] + kappa[i] * sqrt(CN[i])
    for i in 0..n {
        let at = atoms[i].number as usize - 1;
        let cn_i = cn[i].max(0.0); // Ensure non-negative CN
        let tmp = EEQ_KAPPA[at] / (cn_i.sqrt() + 1e-14);
        xvec[i] = -EEQ_EN[at] + tmp * cn_i;
    }

    // Build the Coulomb matrix A
    for i in 0..n {
        let atom_i = &atoms[i];
        let at_i = atom_i.number as usize - 1;
        let alpha_i = EEQ_ALPHA[at_i];

        // Diagonal: A[i,i] = gam[i] + sqrt(2/π) / alpha[i]
        amat[[i, i]] = EEQ_GAM[at_i] + SQRT2PI / alpha_i;

        // Off-diagonal: A[i,j] = erf(γij * r_ij) / r_ij
        for j in 0..i {
            let atom_j = &atoms[j];
            let at_j = atom_j.number as usize - 1;
            let alpha_j = EEQ_ALPHA[at_j];

            // Distance between atoms
            let diff = atom_i.xyz - atom_j.xyz;
            let r_ij = diff.norm();

            // γij = 1 / sqrt(alpha_i² + alpha_j²)
            let gamij = 1.0 / (alpha_i * alpha_i + alpha_j * alpha_j).sqrt();

            // Screened Coulomb: erf(γij * r_ij) / r_ij
            let erf_val = libm::erf(gamij * r_ij);
            let coulomb = erf_val / r_ij;

            amat[[i, j]] = coulomb;
            amat[[j, i]] = coulomb; // Symmetric
        }
    }

    // Add charge conservation constraint
    // Last row: sum(q) = total_charge
    // Last column: Lagrange multiplier
    for i in 0..n {
        amat[[m - 1, i]] = 1.0;
        amat[[i, m - 1]] = 1.0;
    }
    amat[[m - 1, m - 1]] = 0.0;
    xvec[m - 1] = total_charge;

    // Solve the linear system A·x = b using LU decomposition
    match amat.solve(&xvec) {
        Ok(solution) => {
            // Extract charges (first n elements, last element is Lagrange multiplier)
            let mut charges = Array1::zeros(n);
            for i in 0..n {
        ...
        Err(_) => {
            // If solve fails, return zero charges as fallback
            eprintln!("Warning: EEQ linear solve failed, using zero charges");
            Array1::zeros(n)
        }
    }
}

// Note: Tests removed as they require complex XtbAtom initialization
// The EEQ model is tested through integration tests with the full SCC routine
