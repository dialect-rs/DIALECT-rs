...
use crate::scc::{gamma_approximation::*, outer_sum};
use crate::scc::{get_electronic_energy_gamma_shell_resolved, h0_and_s::*};
...
impl Pair<'_> {
    pub fn prepare_scc(
        ...
        m1: &Monomer,
        m2: &Monomer,
        gamma_ao_full: Option<ArrayView2<f64>>,
        shell_resolved: bool,
    ) {
        ...
        let (gamma, gamma_ab): (Array2<f64>, Array2<f64>) = if !shell_resolved {
            ...
            gamma_tmp.slice_mut(s![0..a, a..]).assign(&gamma_ab);
            gamma_tmp.slice_mut(s![a.., 0..a]).assign(&gamma_ab.t());
            (gamma_tmp, gamma_ab)
        } else {
            let gamma_ao_full: ArrayView2<f64> = gamma_ao_full.unwrap();
            let mut gamma_ao: Array2<f64> = Array2::zeros([self.n_orbs, self.n_orbs]);
            let gamma_ab: ArrayView2<f64> = gamma_ao_full.slice(s![m1.slice.orb, m2.slice.orb]);
            gamma_ao.slice_mut(s![..mu, mu..]).assign(&gamma_ab);
            gamma_ao
                .slice_mut(s![..mu, ..mu])
                .assign(&gamma_ao_full.slice(s![m1.slice.orb, m1.slice.orb]));
            gamma_ao
                .slice_mut(s![mu.., mu..])
                .assign(&gamma_ao_full.slice(s![m2.slice.orb, m2.slice.orb]));
            gamma_ao.slice_mut(s![mu.., ..mu]).assign(&gamma_ab.t());
            (gamma_ao, gamma_ab.to_owned())
        };
        ...
        let esp: Array1<f64> = if !shell_resolved {
            let mut tmp = Array1::zeros([self.n_atoms]);
            tmp.slice_mut(s![0..a]).assign(
                &(&m1.properties.esp_q().unwrap() - &(gamma_ab.dot(&m2.properties.dq().unwrap()))),
            );
            tmp.slice_mut(s![a..]).assign(
                &(&m2.properties.esp_q().unwrap()
                    - &(gamma_ab.t().dot(&m1.properties.dq().unwrap()))),
            );
            tmp
        } else {
            let mut tmp = Array1::zeros([self.n_orbs]);
            tmp.slice_mut(s![0..mu]).assign(
                &(&m1.properties.esp_q().unwrap() - &(gamma_ab.dot(&m2.properties.dq().unwrap()))),
            );
            tmp.slice_mut(s![mu..]).assign(
                &(&m2.properties.esp_q().unwrap()
                    - &(gamma_ab.t().dot(&m1.properties.dq().unwrap()))),
            );
            tmp
        ...
        let omega: Array2<f64> = if !shell_resolved {
            atomvec_to_aomat(esp.view(), self.n_orbs, atoms)
        } else {
            aovec_to_aomat(esp.view(), self.n_orbs)
        };
        ...
        if !self.properties.contains_key("dq") {
            ...
            self.properties.set_dq_ao(concatenate![
                Axis(0),
    ...
    pub fn run_scc(&mut self, atoms: &[Atom], config: SccConfig, shell_resolved: bool) {
        ...
        let gamma: ArrayView2<f64> = if !shell_resolved {
        ...
        'scf_loop: for iter in 0..max_iter {
            let h_coul: Array2<f64> = if !shell_resolved {
                atomvec_to_aomat(gamma.dot(&dq).view(), self.n_orbs, atoms) * s * 0.5
            } else {
                outer_sum(gamma.dot(&dq).view()) * s * 0.5
            };
...
impl ESDPair<'_> {
    ...
    pub fn run_scc_lc(&mut self, atoms: &[Atom], config: SccConfig) {
        ...
        // Anderson mixer
        let mix_config: MixConfig = MixConfig::default();
        let dim: usize = if self.gammafunction_lc.is_some() {
