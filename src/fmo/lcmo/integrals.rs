use crate::excited_states::{trans_charges, trans_charges_reduced};
use crate::fmo::helpers::get_pair_slice;
use crate::fmo::lcmo::lcmo_trans_charges::ElecHole;
use crate::fmo::{
    BasisState, ChargeTransferPair, LocallyExcited, Monomer, Pair, PairType, SuperSystem, LRC,
};
...
impl SuperSystem<'_> {
            ...
            || type_le_l == PairType::None
        {
            let q_ij = if i.monomer.index == j.m_h {
                ...
                let q_oo_3d = if restrict_space {
                    let q_oo_3d: Array3<f64> = q_oo
                        .into_shape([i.monomer.n_atoms, nocc, nocc])
                        .unwrap()
                        .to_owned();
                    let dim_occ: usize = i.occ_indices.len();
                    let dim_occ2: usize = j.occ_indices.len();
                    let mut arr: Array3<f64> =
                        Array3::zeros((i.monomer.n_atoms, dim_occ, dim_occ2));
                    ...
                    q_oo.into_shape([i.monomer.n_atoms, nocc, nocc])
                        .unwrap()
                        .to_owned()
                };
            ...
            let q_ab = if i.monomer.index == j.m_l {
                ...
                let q_vv_3d = if restrict_space {
                    let q_vv_3d: Array3<f64> = q_vv
                        .into_shape([i.monomer.n_atoms, nvirt, nvirt])
                        .unwrap()
                        .to_owned();
                    ...
                    let mut arr: Array3<f64> =
                        Array3::zeros((i.monomer.n_atoms, dim_virt, dim_virt2));
                    ...
                    q_vv.into_shape([i.monomer.n_atoms, nvirt, nvirt])
                        .unwrap()
                        .to_owned()
                };
                q_vv_3d
    ...
    pub fn ct_ct<'a>(&self, state_1: &ChargeTransferPair, state_2: &ChargeTransferPair) -> f64 {
        ...
        let exchange: f64 =
            if (type_hh == PairType::Pair && type_ll == PairType::Pair)
                ...
                || (type_hh == PairType::None && type_ll == PairType::Pair)
            {
                ...
                let q_ab: Array3<f64> = if state_1.m_l == state_2.m_l {
                    ...
                    let n_atoms: usize = self.monomers[state_1.m_l].n_atoms;
                    ...
                    let q_vv_3d = if restrict_space {
                        ...
                        let mut arr: Array3<f64> = Array3::zeros((n_atoms, dim_virt, dim_virt2));
