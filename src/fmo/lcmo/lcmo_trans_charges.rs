...
impl SuperSystem<'_> {
    ...
    pub fn q_lect_ao<'a>(
        ...
        match kind {
            ElecHole::Hole => {
                // Number of atoms.
                let n_orbs_le: usize = a.monomer.n_orbs;
                let n_orbs_ct_occ: usize = self.monomers[b.m_h].n_orbs;
                let n_orbs_occ: usize = n_orbs_le + n_orbs_ct_occ;
                ...
                // The transition charges between the two sets of MOs are initialized.
                let mut q_trans_ij: Array3<f64> = Array3::zeros([n_orbs_occ, dim_i, dim_j]);

                // Matrix products between the overlap matrix and the molecular orbitals
                let s_ij_c_ct_occ: Array2<f64> = s_ij.dot(&occs_ct);
                let s_ij_c_le_occ: Array2<f64> = s_ij.t().dot(&occs_le);

                for (idx, mut q_n) in q_trans_ij
                    .slice_mut(s![0..n_orbs_le, .., ..])
                    .axis_iter_mut(Axis(0))
                    .enumerate()
                {
                    for (orb_i, mut q_i) in occs_le.row(idx).iter().zip(q_n.axis_iter_mut(Axis(0)))
                    {
                        for (sc, q) in s_ij_c_ct_occ.row(idx).iter().zip(q_i.iter_mut()) {
                            *q += orb_i * sc;
                        }
                    }
                }
                ...
                for (idx, mut q_n) in q_trans_ij
                    .slice_mut(s![n_orbs_le.., .., ..])
                    .axis_iter_mut(Axis(0))
                    .enumerate()
                {
                    for (sc, mut q_i) in s_ij_c_le_occ
                        .row(idx)
                        .iter()
                        .zip(q_n.axis_iter_mut(Axis(0)))
                    {
                        for (orb_j, q) in occs_ct.row(idx).iter().zip(q_i.iter_mut()) {
                            *q += orb_j * sc;
                        }
                    }
                }
            ...
            ElecHole::Electron => {
                ...
                // The transition charges between the two sets of MOs are initialized.
                let mut q_trans_ab: Array3<f64> = Array3::zeros([n_orbs_virt, dim_a, dim_b]);

                // Matrix products between the overlap matrix and the molecular orbitals
                let s_ab_c_ct_virt: Array2<f64> = s_ab.dot(&virts_ct);
                let s_ab_c_le_virt: Array2<f64> = s_ab.t().dot(&virts_le);

                for (idx, mut q_n) in q_trans_ab
                    .slice_mut(s![0..n_orbs_le, .., ..])
                    .axis_iter_mut(Axis(0))
                    .enumerate()
                {
                    for (orb_i, mut q_i) in virts_le.row(idx).iter().zip(q_n.axis_iter_mut(Axis(0)))
                    {
                        for (sc, q) in s_ab_c_ct_virt.row(idx).iter().zip(q_i.iter_mut()) {
                            *q += orb_i * sc;
                        }
                    }
                }
                    ...
                    .enumerate()
                {
                    for (sc, mut q_i) in s_ab_c_le_virt
                        ...
                        .zip(q_n.axis_iter_mut(Axis(0)))
                    {
                        for (orb_j, q) in virts_ct.row(idx).iter().zip(q_i.iter_mut()) {
                            *q += orb_j * sc;
                        }
                    }
                }
    ...
    pub fn q_ctct_ao(
        ...
        match kind {
            ElecHole::Hole => {
                // Number of atoms.
                let n_orbs_ct_a: usize = self.monomers[a.m_h].n_orbs;
                let n_orbs_ct_b: usize = self.monomers[b.m_h].n_orbs;
                let n_orbs_occ: usize = n_orbs_ct_a + n_orbs_ct_b;
                ...
                for (idx, mut q_n) in q_trans_ij
                    .slice_mut(s![0..n_orbs_ct_a, .., ..])
                    .axis_iter_mut(Axis(0))
                    .enumerate()
                {
                    for (orb_i, mut q_i) in
                        occs_ct_a.row(idx).iter().zip(q_n.axis_iter_mut(Axis(0)))
                    {
                        for (sc, q) in s_ij_c_ct_b.row(idx).iter().zip(q_i.iter_mut()) {
                            *q += orb_i * sc;
                        }
                    }
                }
                ...
                for (idx, mut q_n) in q_trans_ij
                    .slice_mut(s![n_orbs_ct_a.., .., ..])
                    .axis_iter_mut(Axis(0))
                    .enumerate()
                {
                    for (sc, mut q_i) in s_ij_c_ct_a.row(idx).iter().zip(q_n.axis_iter_mut(Axis(0)))
                    {
                        for (orb_j, q) in occs_ct_b.row(idx).iter().zip(q_i.iter_mut()) {
                            *q += orb_j * sc;
                        }
                    }
                }
            ...
            ElecHole::Electron => {
                // Number of atoms.
                let n_orbs_ct_a: usize = self.monomers[a.m_l].n_orbs;
                let n_orbs_ct_b: usize = self.monomers[b.m_l].n_orbs;
                let n_orbs_virt: usize = n_orbs_ct_a + n_orbs_ct_b;
                ...
                {
                    for (orb_i, mut q_i) in
                        virts_ct_a.row(idx).iter().zip(q_n.axis_iter_mut(Axis(0)))
                    {
                        for (sc, q) in s_ab_c_ct_b.row(idx).iter().zip(q_i.iter_mut()) {
                            *q += orb_i * sc;
                        }
                    }
                }
                ...
                for (idx, mut q_n) in q_trans_ab
                    ...
                    .enumerate()
                {
                    for (sc, mut q_i) in s_ab_c_ct_a.row(idx).iter().zip(q_n.axis_iter_mut(Axis(0)))
                    {
                        for (orb_j, q) in virts_ct_b.row(idx).iter().zip(q_i.iter_mut()) {
                            *q += orb_j * sc;
                        }
                    }
                }
    ...
    pub fn q_lele_ao<'a>(
        ...
        // Number of atoms.
        let n_orbs_i: usize = a.monomer.n_orbs;
        let n_orbs_j: usize = b.monomer.n_orbs;
        let n_orbs: usize = n_orbs_i + n_orbs_j;
        // Check if the occupied or virtual orbitals of the first LE state are needed.
        let orbs_i: Array2<f64> = match kind_a {
            ElecHole::Hole => {
                if self.config.fmo_lc_tddftb.restrict_active_space {
        ...
        let dim_j: usize = orbs_j.ncols();
        // The transition charges between the two sets of MOs  are initialized.
        let mut q_trans: Array3<f64> = Array3::zeros([n_orbs, dim_i, dim_j]);
        // Matrix product of overlap matrix with the orbitals on I.
        let sc_mu_j: Array2<f64> = s.dot(&orbs_j);
        // Matrix product of overlap matrix with the orbitals on J.
        let sc_mu_i: Array2<f64> = s.t().dot(&orbs_i);
        ...
        // Iteration over all atoms (I).
        for (idx, mut q_n) in q_trans
            .slice_mut(s![0..n_orbs_i, .., ..])
            .axis_iter_mut(Axis(0))
            .enumerate()
        {
            // Iteration over orbitals i on monomer I. orb_i -> C_(mu i) (mu on I, i on I)
            for (orb_i, mut q_i) in orbs_i.row(idx).iter().zip(q_n.axis_iter_mut(Axis(0))) {
                // Iteration over S * C_J on monomer J. sc -> SC_(mu j) (mu on I, j on J)
                for (sc, q) in sc_mu_j.row(idx).iter().zip(q_i.iter_mut()) {
                    // The transition charge is computed.
                    *q += orb_i * sc;
                }
            }
        ...
        // Iteration over all atoms J.
        for (idx, mut q_n) in q_trans
            .slice_mut(s![n_orbs_i.., .., ..])
            .axis_iter_mut(Axis(0))
            .enumerate()
        {
            // Iteration over occupied orbital i. sc -> SC_(mu i) (mu on J, i on I)
            for (sc, mut q_i) in sc_mu_i.row(idx).iter().zip(q_n.axis_iter_mut(Axis(0))) {
                // Iteration over occupied orbital j. C_(mu j) (mu on J, j on J)
                for (orb_j, q) in orbs_j.row(idx).iter().zip(q_i.iter_mut()) {
                    // The transition charge is computed.
                    *q += orb_j * sc;
                }
            }
