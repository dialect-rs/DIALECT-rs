...
use crate::gradients::helpers::{
    ...
    compute_fv_dg_contributions, f_lr, f_v, zvector_lc, zvector_lc_optimized, Hav, Hplus,
    HplusType,
};
...
use crate::scc::gamma_approximation::{
    gamma_ao_wise, gamma_ao_wise_from_gamma_atomwise, gamma_atomwise_ab,
};
...
impl SuperSystem<'_> {
    pub fn charge_transfer_pair_gradient(&self, ct_state: &ChargeTransferPair) -> Array1<f64> {
        ...
        let ct_gradient: Array1<f64> = if pair_type == PairType::Pair {
            ...
            pair_ij.tda_gradient_lc_accumulation(&pair_atoms, 0)
...
impl Pair<'_> {
    ...
    /// On-the-fly accumulation version of tda_gradient_lc for Pair.
...
impl ESDPair<'_> {
    ...
    /// On-the-fly accumulation version of tda_gradient_lc for ESDPair.
    /// Computes H0/S/gamma gradients on-the-fly instead of storing/loading large 3D arrays.
    pub fn tda_gradient_lc_accumulation(&self, atoms: &[Atom], state: usize) -> Array1<f64> {
        // get occ and virt indices from properties
        let occ_indices: &[usize] = self.properties.occ_indices().unwrap();
