use crate::excited_states::casida::b_engine::BsolverEngine;
...
pub struct BsolverError;

impl fmt::Display for BsolverError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Davidson Algorithm for B matrix did not converge!")
    }
}

impl error::Error for BsolverError {}
...
impl Bsolver {
    ...
    /// * `engine` an object that implements the `BsolverEngine` trait.
    ...
    pub fn new<D: BsolverEngine>(
    ...
    ) -> Result<Self, BsolverError> {
        // Timer to measure the time within the Bsolver routine.
        ...
        // The initial information of the Bsolver routine are printed.
        print_str += &utils::print_davidson_init(max_iter, n_roots, tolerance);

        // Initialization of the result.
        let mut result = Err(BsolverError);
