...
impl Properties {
    ...
    pub fn take_mixer_new(&mut self) -> Result<BroydenMixerNew, Property> {
        match self.take("mixer") {
            Some(value) => value.into_mixer_new(),
            _ => Err(Property::default()),
        }
    }
    ...
    pub fn take_hop_projector(&mut self) -> Option<Array2<f64>> {
        self.take("hop_projector")
            .and_then(|value| value.into_array2().ok())
    }
