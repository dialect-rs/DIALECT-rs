...
impl Properties {
    ...
    pub fn reset_reduced(&mut self) {
        ...
        if let Some(old_nacv) = self.old_nacv() {
            new_map.insert("old_nacv", Property::from(old_nacv.to_owned().to_vec()));
        }
    ...
    /// For SP jobs: remove heavy trimer properties that are only needed for gradients.
    /// Keeps only last_energy, delta_dq, dq, and dq_ao.
    pub fn reset_trimer_sp(&mut self) {
        let keys_to_remove = [
            "H0",
            "S",
            "gamma_ao_wise",
    ...
    /// Remove intermediate SCC properties that are not needed after convergence.
    /// Keeps S, gamma_ao, P, orbs, orbe, occupation, dq, dq_ao, delta_dq, last_energy, esp_q,
    /// occ_indices, virt_indices which are needed for gradient calculations.
    pub fn reset_scc_intermediates(&mut self) {
        let keys_to_remove = ["H0", "X", "V", "ref_density_matrix", "mixer_new"];
        for key in &keys_to_remove {
            self.map.remove(*key);
        }
    }
