...
impl Properties {
    ...
    pub fn old_orbs(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("old_orbs")
            .map(|value| value.as_array2().unwrap().view())
    }

    pub fn old_ci_coeffs(&self) -> Option<ArrayView3<'_, f64>> {
    ...
    /// Returns the stored number of electrons (may differ from sum of atom.n_elec due to HOP).
    pub fn n_elec(&self) -> Option<usize> {
        self.get("n_elec").map(|value| *value.as_usize().unwrap())
    }

    /// Returns the CI coefficients (in MO basis) for a specific excited state.
    pub fn q_trans(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("q_trans")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns the Mulliken charge for every atomic orbital
    pub fn q_ao(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("q_ao")
            .map(|value| value.as_array1().unwrap().view())
    }

    /// Returns the transition dipole moments for all excited states.
    pub fn tr_dipoles(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("tr_dipoles")
            .map(|value| value.as_array2().unwrap().view())
    }
    ...
    pub fn oscillator_strengths(&self) -> Option<ArrayView1<'_, f64>> {
    ...
    pub fn ci_coefficient(&self, idx: usize) -> Option<ArrayView1<'_, f64>> {
    ...
    /// Returns the CI coefficients (in MO basis) for a specific excited state.
    /// The first excited state has the index 0.
    pub fn ci_coefficient_b(&self, idx: usize) -> Option<ArrayView1<'_, f64>> {
        self.get("ci_coefficients_b")
            .map(|value| value.as_array2().unwrap().column(idx))
    }

    /// Returns the CI coefficients (in MO basis) for all computed states.
    pub fn ci_coefficients(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("ci_coefficients")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns the CI coefficients (in MO basis) for all computed states.
    pub fn ci_coefficients_b(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("ci_coefficients_b")
            .map(|value| value.as_array2().unwrap().view())
    }
    ...
    /// The first excited state has the index 0.
    pub fn tdm(&self, idx: usize) -> Option<ArrayView2<'_, f64>> {
    ...
    /// Returns the 1-particle transition density matrix (in MO basis) for an excited state.
    /// The first excited state has the index 0.
    pub fn tdm_b(&self, idx: usize) -> Option<ArrayView2<'_, f64>> {
        let n_occ: usize = self.occ_indices().unwrap().len();
        let n_virt: usize = self.virt_indices().unwrap().len();
    ...
    pub fn gw_screend_eris(&self, idx: usize) -> Option<ArrayView2<'_, f64>> {
        let n_occ: usize = self.occ_indices().unwrap().len();
        let n_virt: usize = self.virt_indices().unwrap().len();
    ...
    pub fn ci_eigenvalues(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("ci_eigenvalues")
            .map(|value| value.as_array1().unwrap().view())
    }

    /// Returns a reference to the excitation energies for all excited states.
    pub fn ci_eigenvalues_b(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("ci_eigenvalues_b")
            .map(|value| value.as_array1().unwrap().view())
    }
    ...
    pub fn tddft_eigenvalues(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("tddft_eigenvalues")
            .map(|value| value.as_array1().unwrap().view())
    }
    ...
    pub fn p_ref(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("ref_density_matrix")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to LCMO Fock matrix.
    pub fn lcmo_fock(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("lcmo_fock")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the H0 matrix in AO basis.
    pub fn h0(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("H0")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the MO coefficients.
    pub fn orbs(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("orbs")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the MO coefficients.
    pub fn orbs_slice(&self, start: usize, end: Option<usize>) -> Option<ArrayView2<'_, f64>> {
        match self.get("orbs") {
    ...
    /// resides.
    pub fn grad_dq(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("grad_dq")
            .map(|value| value.as_array2().unwrap().view())
    }
    ...
    pub fn grad_dq_diag(&self) -> Option<ArrayView1<'_, f64>> {
    ...
    pub fn z_vector(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("z_vector")
            .map(|value| value.as_array1().unwrap().view())
    }

    /// Returns a reference to the esp charges per atom
    pub fn esp_q(&self) -> Option<ArrayView1<'_, f64>> {
    ...
    pub fn v(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("V").map(|value| value.as_array2().unwrap().view())
    }

    /// Returns the HOP projector matrix if present.
    pub fn hop_projector(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("hop_projector")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the density matrix in AO basis.
    pub fn p(&self) -> Option<ArrayView2<'_, f64>> {
    ...
    pub fn p_alpha(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("P_alpha")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the density matrix of the beta electrons in AO basis.
    pub fn p_beta(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("P_beta")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the difference density matrix in AO basis.
    pub fn delta_p(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("diff_density_matrix")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the gamma matrix in atomic basis.
    pub fn gamma(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_atom_wise")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the gamma matrix in atomic basis for the third order interactions.
    pub fn gamma_third_order(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_third_order")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a slice of the gamma matrix in atomic basis.
    pub fn gamma_slice(&self, rows: Slice, cols: Slice) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_atom_wise")
            .map(|value| value.as_array2().unwrap().slice(s![rows, cols]))
    }

    pub fn gamma_shell(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_shell_wise")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the gamma matrix in AO basis.
    pub fn gamma_ao(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_ao_wise")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a slice of the gamma matrix in AO basis.
    pub fn gamma_ao_slice(&self, rows: Slice, cols: Slice) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_ao_wise")
            .map(|value| value.as_array2().unwrap().slice(s![rows, cols]))
    }

    /// Returns a reference to the long-range corrected gamma matrix in atomic basis.
    pub fn gamma_lr(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_lr_atom_wise")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a slice of the gamma matrix in atomic basis.
    pub fn gamma_lr_slice(&self, rows: Slice, cols: Slice) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_lr_atom_wise")
            .map(|value| value.as_array2().unwrap().slice(s![rows, cols]))
    }

    /// Returns a slice of the gamma matrix in AO basis.
    pub fn gamma_lr_ao_slice(&self, rows: Slice, cols: Slice) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_lr_ao_wise")
            .map(|value| value.as_array2().unwrap().slice(s![rows, cols]))
    }

    /// Returns a reference to the long-range corrected gamma matrix in AO basis.
    pub fn gamma_lr_ao(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_lr_ao_wise")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the cached gamma_lr^T . q_vv contraction for CT Davidson.
    /// Cached LC exchange tensor (gamma_lr . q_vv), permuted to standard
    /// (n_virt, n_virt, n_atoms) layout for the Davidson matvec.
    pub fn gamma_lr_qvv_perm(&self) -> Option<ArrayView3<'_, f64>> {
        self.get("gamma_lr_qvv_perm_cache")
            .map(|value| value.as_array3().unwrap().view())
    }

    /// Cached shell-resolved LC exchange matrix (gamma_lr_ao . q_oo),
    /// reshaped to (n_orbs * n_occ, n_occ) for the Davidson matvec.
    pub fn gamma_lr_qoo_ao(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("gamma_lr_qoo_ao_cache")
            .map(|value| value.as_array2().unwrap().view())
    }

    pub fn gamma_lr_qvv(&self) -> Option<ArrayView3<'_, f64>> {
        self.get("gamma_lr_qvv_cache")
            .map(|value| value.as_array3().unwrap().view())
    }

    /// Returns a reference to the gradient of the gamma matrix in atomic basis.
    pub fn grad_gamma(&self) -> Option<ArrayView3<'_, f64>> {
        self.get("gamma_atom_wise_gradient")
            .map(|value| value.as_array3().unwrap().view())
    }

    /// Returns a reference to the gradient of the gamma matrix in AO basis.
    pub fn grad_gamma_ao(&self) -> Option<ArrayView3<'_, f64>> {
    ...
    pub fn grad_gamma_lr(&self) -> Option<ArrayView3<'_, f64>> {
    ...
    pub fn grad_gamma_lr_ao(&self) -> Option<ArrayView3<'_, f64>> {
        self.get("gamma_lr_ao_wise_gradient")
            .map(|value| value.as_array3().unwrap().view())
    }

    /// Get an overlap matrix between two monomers
    pub fn overlap_ij(&self, i: usize, j: usize) -> ArrayView2<'_, f64> {
        if i == j {
    ...
    pub fn gamma_ij(&self, i: usize, j: usize) -> ArrayView2<'_, f64> {
        if i == j {
            let map: &HashMap<(usize, usize), Array2<f64>> =
    ...
    /// Get a gamma matrix between two monomers
    pub fn gamma_lr_ij(&self, i: usize, j: usize) -> ArrayView2<'_, f64> {
        if i == j {
            let map: &HashMap<(usize, usize), Array2<f64>> =
                self.get("gamma_lr_map").unwrap().as_matrix_map().unwrap();
            map.get(&(i, i)).unwrap().view()
    ...
    pub fn type_of_pair(&self, i: usize, j: usize) -> PairType {
        if i == j {
            ...
            map.get(&(i, j))
                .or_else(|| map.get(&(j, i)))
                .copied()
                .unwrap_or(PairType::ESD)
        }
    }

    /// Get type of a monomer pair.
    /// Returns ESD by default if the pair is not found (only close pairs are stored).
    pub fn type_of_pair_reduced(&self, i: usize, j: usize) -> PairType {
        if i == j {
            PairType::None
        } else {
            let map: &HashMap<(usize, usize), PairType> =
    ...
    pub fn coupling_signs(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("coupling_signs")
            .map(|value| value.as_array1().unwrap().view())
    }

    pub fn u_matrix(&self) -> Option<ArrayView3<'_, f64>> {
        self.get("u_matrix")
            .map(|value| value.as_array3().unwrap().view())
    }

    pub fn orbs_derivative(&self) -> Option<ArrayView3<'_, f64>> {
    ...
    /// Sign-tracked NACV vectors from the previous dynamics step.
    pub fn old_nacv(&self) -> Option<&Vec<Array1<f64>>> {
        match self.get("old_nacv") {
            Some(value) => Some(value.as_vec_array1().unwrap()),
            _ => None,
        }
    }
    ...
    /// Returns the total shift vector (gamma*dq + ESP) for explicit gradient.
    pub fn shift_total(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("shift_total")
            .map(|value| value.as_array1().unwrap().view())
    }

    /// Returns the coordination numbers.
    pub fn cn(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("coordination_numbers")
            .map(|value| value.as_array1().unwrap().view())
    }

    /// Returns the coordination number gradients.
    pub fn cn_grad(&self) -> Option<ArrayView2<'_, f64>> {
        self.get("coordination_number_gradients")
            .map(|value| value.as_array2().unwrap().view())
    }

    /// Returns a reference to the charge differences per shell.
    pub fn dq_shell(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("dq_shell")
            .map(|value| value.as_array1().unwrap().view())
    }

    /// Returns the full (non-truncated) charge differences per shell,
    /// including ghost shell charges from HOP. Falls back to dq_shell
    /// if no full version is stored (non-HOP case).
    pub fn dq_shell_full(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("dq_shell_full")
            .map(|value| value.as_array1().unwrap().view())
            .or_else(|| self.dq_shell())
    }

    /// Returns the full (non-truncated) atom-level charge differences,
    /// including ghost atoms from HOP. Falls back to dq if no full version stored.
    pub fn dq_full(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("dq_full")
            .map(|value| value.as_array1().unwrap().view())
            .or_else(|| self.dq())
    }

    /// Returns a reference to delta_dq at shell level.
    pub fn delta_dq_shell(&self) -> Option<ArrayView1<'_, f64>> {
        self.get("delta_dq_shell")
            .map(|value| value.as_array1().unwrap().view())
    }
}
