...
impl XtbPair<'_> {
    pub fn prepare_scc(
        ...
        // Build the basis set on demand (deferred from pair construction)
        ...
        // build the pair gamma_shell
        let ns1: usize = m1.n_real_shells;
        let ns_pair: usize = self.basis.shells.len();
        if has_ghosts {
            // Compute gamma from scratch (includes ghost-real interactions)
            let gamma_shell_pair: Array2<f64> =
                gamma_matrix_shell(&self.gammafunction, atoms, &self.basis);
            self.properties.set_gamma_shell(gamma_shell_pair);
        } else {
            // Assemble from supersystem gamma blocks (real shells only)
            let mut gamma_shell_pair: Array2<f64> = Array2::zeros([ns_pair, ns_pair]);
        ...
        let esp_shell_1: Array1<f64> =
            &m1.properties.esp_q().unwrap() - &gamma_ab_shell.dot(&m2_dq_shell);
        let esp_shell_2: Array1<f64> =
            &m2.properties.esp_q().unwrap() - &gamma_ab_shell.t().dot(&m1_dq_shell);
        ...
        esp.slice_mut(s![dim1..dim1 + dim2]).assign(&shell_to_ao_values(
            &real_basis_j,
            m2.n_real_orbs,
            esp_shell_2.view(),
        ));
        // Ghost orbs at dim1+dim2.. remain zero (no external ESP on ghosts)
        ...
        if !self.properties.contains_key("dq") {
            ...
            if has_ghosts {
                ...
                dq_ao = dq_ao_ext;
                let mut dq_shell_ext = Array1::zeros(ns_pair);
    ...
    pub fn run_scc(&mut self, atoms: &[XtbAtom], config: &Configuration) {
        ...
        'scf_loop: for i in 0..max_iter {
            // calculate the coulomb part of the Hamiltonian (shell-level)
