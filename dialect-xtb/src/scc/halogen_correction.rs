...
pub fn get_halogen_correction(atoms: &[XtbAtom]) -> f64 {
    ...
    if halogens.is_empty() || acceptors.is_empty() {
        ...
        let energy: f64 = calculate_halogen_bond_energy(&atoms, &bond_vec);
...
pub fn check_halogen_donor(atoms: &[XtbAtom]) -> Vec<usize> {
...
pub fn check_acceptor_atoms(atoms: &[XtbAtom]) -> Vec<usize> {
...
pub fn get_halogen_bond_indices(
    atoms: &[XtbAtom],
    halogens: &[usize],
    acceptors: &[usize],
