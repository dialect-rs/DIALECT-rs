...
pub fn calculate_h0_gradient_xtb1_new(
    ...
    // iterate over shells (shell-pair level is the outer loop for efficiency)
    for shell_i in basis.shells.iter() {
        // get atom data once per shell_i
        ...
        for shell_j in basis.shells.iter() {
            // get atom data once per shell_j
            ...
            // Precompute shell-pair data ONCE (outside orbital loops)
            ...
            // For off-diagonal elements, precompute all shell-pair constants
            let (scaling_constant, en_term, hueckel_const, pi_term, pi_grad_opt) =
                if shell_i.sph_start != shell_j.sph_start || shell_i.sph_end != shell_j.sph_end {
                    ...
                    // Precompute pi gradient if needed (only for different atoms)
                    let pg = if shell_i.atom_index != shell_j.atom_index {
                        Some(get_pi_term_gradient(
                            &r_vector,
                            distance,
                            atomi.number as usize,
                            atomj.number as usize,
                    ...
                    (0.0, 0.0, 0.0, 0.0, None) // Not used for diagonal
                };

            // Precompute CN gradient contributions for this shell pair
            ...
            // Precompute combined h0 value for off-diagonal
            let h0_val: f64 = scaling_constant * hueckel_const * self_energy_term * en_term * pi_term;
            let h_val_cn: f64 = scaling_constant * hueckel_const * en_term * pi_term;
            let cn_grad_sum: Array1<f64> = &cn_grad_val_i + &cn_grad_val_j;

            // Precompute pi contribution factor
            let pi_factor: f64 = scaling_constant * hueckel_const * self_energy_term * en_term;

            // Now iterate over orbital pairs within shells
            for idx_i in shell_i.sph_start..shell_i.sph_end {
                for idx_j in shell_j.sph_start..shell_j.sph_end {
                    if idx_i == idx_j {
                        ...
                        // CN gradient contribution for diagonal
                        let s_val: f64 = s[[idx_i, idx_j]];
                        let grad_val: Array1<f64> = &cn_grad_val_i * s_val;
                        h0_grad.slice_mut(s![.., idx_i, idx_j]).add_assign(&grad_val);
                    } else {
                        // Off-diagonal: use precomputed shell-pair values
                        let grad_contribution: Array1<f64> =
                            h0_val * &grad_s.slice(s![.., idx_i, idx_j]);
                        h0_grad.slice_mut(s![.., idx_i, idx_j]).add_assign(&grad_contribution);

                        // Pi gradient contribution (only for different atoms)
                        if let Some(ref pi_grad) = pi_grad_opt {
                            let s_ij: f64 = s[[idx_i, idx_j]];
                            let pi_h0_grad_val: Array1<f64> = (pi_factor * s_ij) * pi_grad;
                            h0_grad
                        ...
                        // CN number gradient contribution
                        let s_ij: f64 = s[[idx_i, idx_j]];
                        let grad_contribution: Array1<f64> = (0.5 * h_val_cn * s_ij) * &cn_grad_sum;
                        h0_grad.slice_mut(s![.., idx_i, idx_j]).add_assign(&grad_contribution);
                    }
                }
            }
...
pub fn calculate_h0_gradient_xtb1_atom_specific(
    n_orbs: usize,
    atoms: &[XtbAtom],
    s: ArrayView2<f64>,
    grad_s: ArrayView3<f64>,
    ...
    // init empty array
    let mut h0_grad: Array3<f64> = Array3::zeros([3, n_orbs, n_orbs]);
    // calculate the coordination numbers
    let cn_numbers: Array1<f64> = calculate_coordination_numbers(atoms);
    // get the gradients of the coordination numbers
    let cn_number_grads: Array2<f64> = calculate_coordination_number_gradients(atoms);
    ...
    for shell_i in basis.shells.iter() {
        ...
        for idx_i in (shell_i.sph_start..shell_i.sph_end) {
            ...
            let cn_1: f64 = cn_numbers[shell_i.atom_index];
            let at_i: usize = shell_i.atom_index;
            let cn_grad_i: ArrayView1<f64> =
                cn_number_grads.slice(s![3 * atom_index..3 * atom_index + 3, at_i]);
            ...
            for shell_j in basis.shells.iter() {
                ...
                let cn_2: f64 = cn_numbers[shell_j.atom_index];
                let at_j: usize = shell_j.atom_index;
                let cn_grad_j: ArrayView1<f64> =
                    cn_number_grads.slice(s![3 * atom_index..3 * atom_index + 3, at_j]);
                ...
                if distance < PROXIMITY_CUTOFF {
                    ...
                    for idx_j in (shell_j.sph_start..shell_j.sph_end) {
                        if idx_i == idx_j {
                            ...
                            // xTB applies the element-pair scaling only to valence-valence
                            // shell pairs; pairs involving a polarization shell use 1.0.
                            let scaling_constant: f64 = if shell_i.polarization || shell_j.polarization {
                                1.0
                            } else {
                                calculate_pair_scaling_param(
                                    atomi.number,
...
/// Parallel version of H0 gradient calculation - parallelizes over shell_i
pub fn calculate_h0_gradient_xtb1_parallel(
    n_orbs: usize,
    atoms: &[XtbAtom],
    s: ArrayView2<f64>,
    grad_s: ArrayView3<f64>,
    ...
    // Pre-compute coordination numbers and gradients (shared across all threads)
    let cn_numbers: Array1<f64> = calculate_coordination_numbers(atoms);
    let cn_number_grads: Array2<f64> = calculate_coordination_number_gradients(atoms);

    // Parallel computation over shell_i using fold/reduce pattern
    let h0_grad: Array3<f64> = basis.shells.par_iter()
        .fold(
            || Array3::zeros([3 * n_atoms, n_orbs, n_orbs]),
            |mut h0_grad, shell_i| {
                // get atom data for shell_i
                let atomi: &XtbAtom = &atoms[shell_i.atom_index];
                let cn_1: f64 = cn_numbers[shell_i.atom_index];
                let at_i: usize = shell_i.atom_index;
                let cn_grad_i: ArrayView1<f64> = cn_number_grads.slice(s![.., at_i]);
                ...
                for shell_j in basis.shells.iter() {
                    ...
                    let (scaling_constant, en_term, hueckel_const, pi_term, pi_grad_opt) =
                        if shell_i.sph_start != shell_j.sph_start || shell_i.sph_end != shell_j.sph_end {
                            ...
                            let pauling_diff: f64 = (PAULING_EN[atomi.number as usize - 1]
                                - PAULING_EN[atomj.number as usize - 1]).powi(2);
                            let en = if !shell_i.polarization && !shell_j.polarization {
                                1.0 + EN_SHELL_PARAM * pauling_diff
                            } else { 1.0 };
                            let hc = get_hueckel_constants_new(
                    ...
                    let cn_grad_val_i: Array1<f64> = get_self_energy_value_grad(
                        atomi.number, cn_grad_i.view(), shell_i.shell_index,
                    );
                    let cn_grad_val_j: Array1<f64> = get_self_energy_value_grad(
                        atomj.number, cn_grad_j.view(), shell_j.shell_index,
                    );
                    let h0_val: f64 = scaling_constant * hueckel_const * self_energy_term * en_term * pi_term;
                    let h_val_cn: f64 = scaling_constant * hueckel_const * en_term * pi_term;
                    let cn_grad_sum: Array1<f64> = &cn_grad_val_i + &cn_grad_val_j;
                        ...
                        for idx_j in shell_j.sph_start..shell_j.sph_end {
                            if idx_i == idx_j {
                                ...
                                if let Some(ref pi_grad) = pi_grad_opt {
                                    let s_ij: f64 = s[[idx_i, idx_j]];
                                    let pi_h0_grad_val: Array1<f64> = (pi_factor * s_ij) * pi_grad;
                                    h0_grad.slice_mut(s![at_i * 3..at_i * 3 + 3, idx_i, idx_j])
                                        .add_assign(&pi_h0_grad_val);
                                    h0_grad.slice_mut(s![at_i * 3..at_i * 3 + 3, idx_j, idx_i])
                                        .add_assign(&pi_h0_grad_val);
                                }
...
/// Atom-specific H0 gradient with precomputed CN values
fn calculate_h0_gradient_xtb1_atom_specific_precomputed(
    n_orbs: usize,
    atoms: &[XtbAtom],
    s: ArrayView2<f64>,
    grad_s: ArrayView3<f64>,  // Shape [3, n_orbs, n_orbs] - atom-specific
    basis: &Basis,
    atom_index: usize,
    cn_numbers: ArrayView1<f64>,
    cn_number_grads: ArrayView2<f64>,
) -> Array3<f64> {
    let mut h0_grad: Array3<f64> = Array3::zeros([3, n_orbs, n_orbs]);
                ...
                if distance < PROXIMITY_CUTOFF {
                    for idx_j in shell_j.sph_start..shell_j.sph_end {
                        if idx_i == idx_j {
                            ...
                            let h0_val: f64 = scaling_constant * hueckel_const
                                * self_energy_term * en_term * pi_term;
                            let grad_contribution: Array1<f64> =
                                h0_val * &grad_s.slice(s![.., idx_i, idx_j]);
                            h0_grad.slice_mut(s![.., idx_i, idx_j]).add_assign(&grad_contribution);
                            ...
                            // Pi gradient contribution (only for different atoms)
                            if shell_i.atom_index != shell_j.atom_index {
                                // Check if this atom is involved in the pair
                                if shell_i.atom_index == atom_index || shell_j.atom_index == atom_index {
                                    let pi_grad: Array1<f64> = get_pi_term_gradient(
                                        &r_vector, distance,
                                        atomi.number as usize, atomj.number as usize,
                                        shell_i.angular_momentum, shell_j.angular_momentum,
                                    );
                                    let sign = if shell_i.atom_index == atom_index { 1.0 } else { -1.0 };
                                    let pi_h0_grad_val: Array1<f64> = sign * scaling_constant
                                        * hueckel_const * self_energy_term
                                        * s[[idx_i, idx_j]] * en_term * &pi_grad;
                                    h0_grad.slice_mut(s![.., idx_i, idx_j]).add_assign(&pi_h0_grad_val);
                                    h0_grad.slice_mut(s![.., idx_j, idx_i]).add_assign(&pi_h0_grad_val);
                                }
                            }

                            // CN gradient contribution
                            let cn_grad_val_i: Array1<f64> = get_self_energy_value_grad(
                                atomi.number, cn_grad_i.view(), shell_i.shell_index,
                            );
                            let cn_grad_val_j: Array1<f64> = get_self_energy_value_grad(
                            ...
                            let h_val: f64 = scaling_constant * hueckel_const
                                * en_term * pi_term * s[[idx_i, idx_j]];
                            let grad_contribution: Array1<f64> =
                                0.5 * h_val * &(&cn_grad_val_i + &cn_grad_val_j);
                            h0_grad.slice_mut(s![.., idx_i, idx_j]).add_assign(&grad_contribution);
                        }
