use dialect_dftb_core::geometry::*;
use dialect_config::Configuration;
use dialect_utilities::io::{read_xyz_frame, xyz_frame_to_coordinates};
...
/// Non-FMO GFN1-xTB molecular system holding all data for the SCC and
/// gradient routines (the xTB counterpart of the DFTB `System`).
#[derive(Clone)]
...
impl From<(Vec<u8>, Array2<f64>, Configuration)> for XtbSystem {
    ...
    // the global configuration as [Configuration](dialect_config::settings::Configuration).
...
impl From<(XyzFrame, Configuration)> for XtbSystem {
    // Creates a new [System] from an [XyzFrame](xyz_parser::XyzFrame) and
    // the global configuration as [Configuration](dialect_config::settings::Configuration).
...
impl From<(&str, Configuration)> for XtbSystem {
    fn from(filename_and_config: (&str, Configuration)) -> Self {
        let frame: XyzFrame = read_xyz_frame(filename_and_config.0);
