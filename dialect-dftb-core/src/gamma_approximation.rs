...
impl GammaFunction {
    ...
    /// Evaluate the screened Coulomb interaction γ_AB(r) for the atom pair
    /// (z_a, z_b) at distance r.
    pub fn eval(&self, r: f64, z_a: u8, z_b: u8) -> f64 {
    ...
    /// Analytic r -> 0 limit of the two-centre gamma for a (possibly
    /// hetero) element pair. Needed for HOP extended atom sets, where a
    /// ghost boundary atom deliberately coincides with its bond-attached
    /// atom. Gaussian: lim erf(c r)/r = 2c/sqrt(pi). Slater: the same
    /// closed form the short-range eval uses at r -> 0.
    pub fn eval_limit0_pair(&self, z_a: u8, z_b: u8) -> f64 {
        match *self {
            GammaFunction::Gaussian { ref c, .. } => 2.0 * c[&(z_a, z_b)] / PI.sqrt(),
            GammaFunction::Slater { ref tau, .. } => {
                let t_a: f64 = tau[&z_a];
                let t_b: f64 = tau[&z_b];
                0.5 * ((t_a * t_b) / (t_a + t_b) + (t_a * t_b).powi(2) / (t_a + t_b).powi(3))
            }
            _ => panic!(
                "eval_limit0_pair: coincident-atom gamma limit not implemented \
                 for this gamma variant"
            ),
        }
    }
    ...
    /// Radial derivative dγ_AB/dr for the atom pair (z_a, z_b), used to build
    /// the gamma-matrix gradient.
    pub fn deriv(&self, r: f64, z_a: u8, z_b: u8) -> f64 {
        let result: f64 = match *self {
            GammaFunction::Gaussian {
                hubbards: _,
                sigma: _,
    ...
    /// Shell-resolved radial derivative dγ_{A,l_a; B,l_b}/dr for the atom pair
    /// (z_a, z_b) and the shells (l_a, l_b).
    pub fn deriv_shell_resolved(&self, r: f64, z_a: u8, z_b: u8, l_a: u8, l_b: u8) -> f64 {
        let result: f64 = match *self {
            GammaFunction::GaussianShellResolved {
                sigma: _,
                ref c,
...
/// Same-set atomwise gamma for HOP extended atom lists, which contain
/// ghost boundary atoms coincident with their bond-attached atoms:
/// off-diagonal entries at r ~ 0 use the analytic pair limit instead of
/// the singular two-centre formula. For genuinely distinct atom sets use
/// [`gamma_atomwise`], which keeps the strict r > 0 assertion.
pub fn gamma_atomwise_hop(
    gamma_func: &GammaFunction,
    atoms: &[Atom],
    n_atoms: usize,
) -> Array2<f64> {
    ...
    for (i, atomi) in atoms.iter().enumerate() {
        for (j, atomj) in atoms.iter().enumerate() {
            match i.cmp(&j) {
                ...
                Ordering::Less => {
                    let r = (atomi - atomj).norm();
                    g0[[i, j]] = if r < 1.0e-9 {
                        gamma_func.eval_limit0_pair(atomi.number, atomj.number)
                    } else {
                        gamma_func.eval(r, atomi.number, atomj.number)
                    };
                }
                Ordering::Greater => {
                    g0[[i, j]] = g0[[j, i]];
...
pub fn gamma_gradients_atomwise_full(
    gamma_func: &GammaFunction,
    atoms: &[Atom],
    n_atoms: usize,
) -> Array3<f64> {
...
pub fn gamma_gradients_atomwise_atom_specific(
    gamma_func: &GammaFunction,
    atom_idx: usize,
    atomi: &Atom,
    atoms: &[Atom],
    ...
    let mut g1_val: Array2<f64> = Array2::zeros((n_atoms, n_atoms));
    let mut g1: Array3<f64> = Array3::zeros((3, n_atoms, n_atoms));

    let i: usize = atom_idx;
    for (j, atomj) in atoms.iter().enumerate() {
        match i.cmp(&j) {
            Ordering::Less => {
                let r = atomi - atomj;
...
/// AO-resolved variant of [`gamma_atomwise_hop`] for HOP extended atom
/// lists (ghosts coincident with their bond-attached atoms allowed).
pub fn gamma_ao_wise_hop(
    gamma_func: &GammaFunction,
    atoms: &[Atom],
    n_atoms: usize,
    n_orbs: usize,
) -> (Array2<f64>, Array2<f64>) {
    let g0: Array2<f64> = gamma_atomwise_hop(gamma_func, atoms, n_atoms);
    let mut g0_a0: Array2<f64> = Array2::zeros((n_orbs, n_orbs));
    let mut mu: usize = 0;
    let mut nu: usize;
    for (i, atomi) in atoms.iter().enumerate() {
...
pub fn gamma_gradients_ao_wise_atom_specific(
    gamma_func: &GammaFunction,
    atom_idx: usize,
    atomi: &Atom,
    atoms: &[Atom],
    ...
    let g1: Array3<f64> =
        gamma_gradients_atomwise_atom_specific(gamma_func, atom_idx, atomi, atoms, n_atoms);
    let mut g1_a0: Array3<f64> = Array3::zeros((3, n_orbs, n_orbs));
    let mut mu: usize = 0;
    ...
    for _ in 0..atomi.n_orbs {
        ...
        for (j, atomj) in atoms.iter().enumerate() {
            for _ in 0..atomj.n_orbs {
                if i != j {
                    g1_a0
                        .slice_mut(s![0..3, mu, nu])
                        .assign(&g1.slice(s![0..3, i, j]));
                    g1_a0
                        .slice_mut(s![0..3, nu, mu])
                        .assign(&g1.slice(s![0..3, i, j]));
                }
                nu += 1;
            }
